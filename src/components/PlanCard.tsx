    ...
    return (
        <div className={`p-5 rounded-xl border transition-all hover:shadow-md ${highlight ? 'border-brand-primary bg-brand-primary/5 ring-1 ring-brand-primary/20' : 'border-neutral-200 bg-white'}`}>
            <div className="flex justify-between items-start mb-2">
                ...
                {highlight && <span className="text-xs font-bold text-brand-contrast bg-brand-primary px-2 py-0.5 rounded-full">BEST VALUE</span>}
            </div>

            <div className="mb-1">
                <span className="text-4xl font-extrabold text-brand-text">{(plan.dataMB / (plan.dataMB >= 1000 ? 1024 : 1)).toFixed(plan.dataMB >= 1000 ? 1 : 0)}</span>
            ...
            <div className="w-full bg-neutral-100 rounded-lg p-2 flex items-center justify-between text-xs">
                ...
                <span className="font-bold text-brand-text">{plan.valueScore.toFixed(2)} MB/₦</span>
            </div>
